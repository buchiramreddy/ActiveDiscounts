...
public class BandedPriceFulfillmentTest extends TestCase {
  ...
  public void testFlatRatesExclusive() throws Exception {
    ...
    assertEquals(new Money("45"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("18.00"), 3, new String[] { "10", "15", "20" }, null, option)));
    assertEquals(new Money("5"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("80.00"), 1, new String[] { "5" }, null, option)));
    assertEquals(new Money("10"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("18.00"), 2, new String[] { "8", "2" }, null, option)));
  }
  ...
  public void testFlatRatesWithBands() throws Exception {
    ...
    assertEquals(new Money("35"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("18.00"), 6, new String[] { "10", "15" }, null, option)));
    assertEquals(new Money("125"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("18.00"), 6, new String[] { "5", "100", "20" }, null, option)));
    assertEquals(new Money("41"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("60.00"), 6, new String[] { "8", "2", "1" }, null, option)));
  }
  ...
  public void testLowestPriceSelection() throws Exception {
    ...
    assertEquals(calculationResponse(option, createCandidateOrder(new BigDecimal("10.00"), 2, option)),
      new Money("10.00"));
  }
  ...
  public void testMinimumAmountsWithZero() throws Exception {
    ...
    assertEquals(new Money("20.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("20.00"), 2, option)));
    assertEquals(new Money("10.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("9.00"), 3, option)));
    assertEquals(new Money("30.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("30.00"), 3, option)));
    assertEquals(new Money("20.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("25.00"), 5, option)));
    assertEquals(new Money("30.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("100.00"), 5, option)));
  }
  ...
  public void testPriceBandPercentage() throws Exception {
    ...
    assertEquals(new Money("1.50"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("15.00"), 3, option)));
    assertEquals(new Money("6.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("30.00"), 3, option)));
    assertEquals(new Money("7.50"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("25.00"), 5, option)));
    assertEquals(new Money("20.00"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("100.00"), 5, option)));
  }
  ...
  public void testPriceBandRatesWithPercentages() throws Exception {
    ...
    assertEquals(new Money("20"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("35.00"), 5, option)));
    assertEquals(new Money("20"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("9999.00"), 9, option)));
    assertEquals(new Money("7.50"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("25.00"), 5, option)));
    assertEquals(new Money("1.80"),
      calculationResponse(option, createCandidateOrder(new BigDecimal("18.00"), 6, option)));
  ...
  } // end method testPriceBandRatesWithPercentages
  ...
  public void testWeightBandsWithQuantities() throws Exception {
    ...
    assertEquals(new Money("30"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("18.00"), 3, null, new int[] { 2, 3, 5 }, option)));

    // 66lbs
    assertEquals(new Money("10"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("18.00"), 6, null, new int[] { 4, 1, 2, 5, 5, 5 }, option)));

    // 120lbs
    assertEquals(new Money("20"),
      calculationResponse(option,
        createCandidateOrder(new BigDecimal("60.00"), 3, null, new int[] { 2, 3, 2 }, option)));
  }
  ...
  protected Money calculationResponse(FulfillmentOption option, Order order) throws Exception {
    ...
    return provider.estimateCostForFulfillmentGroup(order.getFulfillmentGroups().get(0), options)
      .getFulfillmentOptionPrices().get(option);
  }
   ...
   * DOCUMENT ME!
   *
   * @param   retailTotal         DOCUMENT ME!
   * @param   orderItemsToCreate  DOCUMENT ME!
   * @param   option              DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   */
   ...
   * DOCUMENT ME!
   *
   * @param   total               - this number divided by the number of items to create is the value of either the
   *                              weight or the price (depending on which <b>option</b> is being passed in) for a single
   *                              order item. Note that the final price of each item will be: (<b>total</b> / <b>
   *                              orderItemsToCreate</b>) * <b>quantity</b>
   * @param   orderItemsToCreate  - the number of order items to split the retail total across
   * @param   flatRates           - the flat rates to assign to the OrderItems that are created. To have an Order that
   *                              is mixed between OrderItems and DiscreteOrderItems (which are created for flat rates)
   *                              ensure that the size of this array is less than <b>orderItemsToCreate</b>
   * @param   quantities          - the quantities to assign to each OrderItem. If specified, this should be equal to
   ...
   */
  protected Order createCandidateOrder(BigDecimal total, int orderItemsToCreate, String[] flatRates, int[] quantities,
    FulfillmentOption option) {
    if ((flatRates != null) && (flatRates.length > orderItemsToCreate)) {
      throw new IllegalStateException(
        "Flat rates for Skus should be less than or equal to the number of order items being created");
    }

    if ((quantities != null) && (quantities.length != orderItemsToCreate)) {
      throw new IllegalStateException(
        "Quantities for Skus should be less than or equal to the number of order items being created");
    }
    ...
    return result;
  } // end method createCandidateOrder

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * Creates price bands with the given minimum amounts, results and result types. All of the lists should be the same
   * size
   *
   * @param   minimumAmounts     DOCUMENT ME!
   * @param   resultAmounts      DOCUMENT ME!
   * @param   resultAmountTypes  DOCUMENT ME!
   *
   * @return  creates price bands with the given minimum amounts, results and result types.
   *
   * @throws  IllegalStateException  DOCUMENT ME!
   */
  protected BandedPriceFulfillmentOption createPriceBands(String[] minimumAmounts, String[] resultAmounts,
    FulfillmentBandResultAmountType[] resultAmountTypes) {
   ...
   * @throws  IllegalStateException  DOCUMENT ME!
   */
  protected BandedWeightFulfillmentOption createWeightBands(String[] minimumAmounts, String[] resultAmounts,
    FulfillmentBandResultAmountType[] resultAmountTypes) {
    if ((minimumAmounts.length != resultAmounts.length) || (resultAmounts.length != resultAmountTypes.length)) {
      throw new IllegalStateException("All lists should be the same length");
    }
...
} // end class BandedPriceFulfillmentTest
