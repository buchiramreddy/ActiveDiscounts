...
public class SolrIndexServiceImpl implements SolrIndexService {
  ...
  /**
   * @see  org.broadleafcommerce.core.search.service.solr.SolrIndexService#rebuildIndex()
   */
  @Override
  ...
  public void rebuildIndex() throws ServiceException, IOException {
    ...
    HashMap                      savedPricing           = SkuPricingConsiderationContext
  ...
  } // end method buildIncrementalIndex
   ...
   * read the products in a different manner. For example, if you know the fields that will be indexed, you can
   * configure a DAO object to only load those fields. You could also use a JDBC based DAO for even faster access. This
   * default implementation is only suitable for small catalogs.
   *
   ...
   *
   * <p>It could still be improved for specific implementations by only loading fields that will be indexed or by
   * accessing the database via direct JDBC (instead of Hibernate).</p>
   *
   * @param   page      DOCUMENT ME!
   * @param   pageSize  DOCUMENT ME!
   *
  ...
  } // end method attachBasicDocumentFields

  /**
   * Returns a map of prefix to value for the requested attributes. For example, if the requested field corresponds to a
   * Sku's description and the locales list has the en_US locale and the es_ES locale, the resulting map could be
   *
   * <p>{ "en_US" : "A description", "es_ES" : "Una descripcion" }</p>
   *
   * @param   product    DOCUMENT ME!
   * @param   field      DOCUMENT ME!
   * @param   fieldType  isPriceField
   * @param   locales    prefix
   *
  ...
  protected Map<String, Object> getPropertyValues(Product product, Field field, FieldType fieldType,
    ...
    if (extensionManager != null) {
      ExtensionResultStatusType result = extensionManager.getProxy().addPropertyValues(product, field, fieldType,
  ...
  /**
   * Converts a propertyName to one that is able to reference inside a map. For example, consider the property in
   * Product that references a List<ProductAttribute>, "productAttributes". Also consider the utility method in Product
   * called "mappedProductAttributes", which returns a map of the ProductAttributes keyed by the name property in the
   * ProductAttribute. Given the parameters "productAttributes.heatRange", "productAttributes",
   * "mappedProductAttributes" (which would represent a property called "productAttributes.heatRange" that references a
   * specific ProductAttribute inside of a product whose "name" property is equal to "heatRange", this method will
   * convert this property to mappedProductAttributes(heatRange).value, which is then usable by the standard beanutils
   * PropertyUtils class to get the value.
   *
   * @param   propertyName      DOCUMENT ME!
   * @param   listPropertyName  DOCUMENT ME!
   * @param   mapPropertyName   DOCUMENT ME!
   *
...
} // end class SolrIndexServiceImpl
