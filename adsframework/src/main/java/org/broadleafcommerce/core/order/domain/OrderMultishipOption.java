...
/**
 * Represents a given set of options for an OrderItem in an Order in the multiship context. This class is used to store
 * current multiship settings for an Order without having to generate the necessary FulfillmentGroups and
 * FulfillmentGroupItems. It also can be used to re-create the multiship set should the Order change
 *
...
public interface OrderMultishipOption {
  ...
  /**
   * Gets the OrderItem associated with this OrderMultishipOption. Note that the default Broadleaf implementation will
   * produce an equal number of instances of OrderMultishipOption to the quantity of the OrderItem
   *
...
} // end interface OrderMultishipOption
