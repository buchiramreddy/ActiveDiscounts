...
public class ValidateAddRequestActivity extends BaseActivity<CartOperationContext> {
  ...
  @Override public CartOperationContext execute(CartOperationContext context) throws Exception {
    ...
    if ((request.getOrder().getCurrency() != null) && (sku.getCurrency() != null)
  ...
  protected Sku determineSku(Product product, Long skuId, Map<String, String> attributeValues) {
    ...
    if ((sku == null) && (product != null)) {
      ...
      if ((product.getAdditionalSkus() != null) && (product.getAdditionalSkus().size() > 0)
            && !product.getCanSellWithoutOptions()) {
        throw new RequiredAttributeNotProvidedException(
          "Unable to find non-default sku matching given options and cannot sell default sku");
      }
      ...
      for (ProductOption productOption : product.getProductOptions()) {
        if (productOption.getRequired()) {
          if (StringUtils.isEmpty(attributeValues.get(productOption.getAttributeName()))) {
            throw new RequiredAttributeNotProvidedException("Unable to add to product (" + product.getId()
              + ") cart. Required attribute was not provided: " + productOption.getAttributeName());
        ...
        if (!productOption.getRequired()
              && StringUtils.isEmpty(attributeValues.get(productOption.getAttributeName()))) {
...
} // end class ValidateAddRequestActivity
