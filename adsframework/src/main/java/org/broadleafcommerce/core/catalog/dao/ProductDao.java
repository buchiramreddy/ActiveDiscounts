...
public interface ProductDao {
  ...
  /**
   * Find all {@code Product} instances whose name starts with or is equal to the passed in search parameter.
   *
  ...
  /**
   * Find all products whose start and end dates are before and after the passed in date and who are related to the
   * given category.
   *
   ...
   */
  @Nonnull List<Product> readActiveProductsByCategory(@Nonnull Long categoryId, @Nonnull Date currentDate);

  /**
   * Find all products whose start and end dates are before and after the passed in date, who are related to the given
   * category, match the given search criteria, and are not marked as archived.
   *
   ...
   */
  @Nonnull List<Product> readFilteredActiveProductsByCategory(Long categoryId, Date currentDate,
    ProductSearchCriteria searchCriteria);

  /**
   * Find all products whose start and end dates are before and after the passed in date, who match the search string,
   * match the given search criteria, and are not marked as archived.
   *
   ...
   * @return  the matching products
   */
  @Nonnull List<Product> readFilteredActiveProductsByQuery(String query, Date currentDate,
    ProductSearchCriteria searchCriteria);
   ...
   */
  @Nonnull List<Product> readActiveProductsByCategory(@Nonnull Long categoryId, @Nonnull Date currentDate,
    @Nonnull int limit, @Nonnull int offset);
   ...
   * extension of {@code Product} by default, include an entity configuration bean in your application context xml
   * similar to:
   *
   * <p>
   ...
   *
   * @param   productType  the type of product you would like to create (presumably a Product or ProductSku instance).
   *                       The getType method of {@code ProductType} provides the key for the entity configuration.
   *
   ...
   *
   * @return  all active ProductBundles whose automatic property is true.
   */
   ...
   *
   * @param   key  uri - the relative URL to look up the Product by
   *
  ...
  /**
   * Reads all products from the database that are currently active. That is, reads all products that are not archived
   * and whose start and end dates surround the currentDate
   *
   * @param   currentDate  DOCUMENT ME!
   *
  ...
  /**
   * Reads all products from the database that are currently active. That is, reads all products that are not archived
   * and whose start and end dates surround the currentDate. This method differs from
   * {@link #readAllActiveProducts(java.util.Date)} in that this one will utilize database paging.
   *
   * <p>It will fetch results in pages. For example, if page = 3 and pageSize = 25, this method would return rows 75-99
   * from the database.</p>
   *
