...
/**
 * <p>Used in conjunction with {@link org.broadleafcommerce.core.order.fulfillment.domain.BandedPriceFulfillmentOption}
 * and {@link org.broadleafcommerce.core.order.fulfillment.domain.BandedWeightFulfillmentOption}. If 2 bands are
 * configured equal to each other (meaning, there are 2
 * {@link org.broadleafcommerce.core.order.fulfillment.domain.FulfillmentPriceBand}s that have the same retail price
 * minimum or 2 {@link org.broadleafcommerce.core.order.fulfillment.domain.FulfillmentWeightBand}s that have the same
 * minimum weight), this will choose the cheaper rate of the 2</p>
 *
 * <p>If the retail total does not fall within a configured price band, the total cost of fulfillment is zero</p>
 *
 * <p>Note: For {@link org.broadleafcommerce.core.order.fulfillment.domain.BandedWeightFulfillmentOption}, this assumes
 * that all of your weights have the same units</p>
 *
 * @author   Phillip Verheyden
 * @see      {@link org.broadleafcommerce.core.order.fulfillment.domain.BandedPriceFulfillmentOption},
 *           {@link org.broadleafcommerce.core.order.fulfillment.domain.FulfillmentPriceBand}
...
public class BandedFulfillmentPricingProvider implements FulfillmentPricingProvider {
  ...
  @Override public FulfillmentGroup calculateCostForFulfillmentGroup(FulfillmentGroup fulfillmentGroup)
    ...
    if (canCalculateCostForFulfillmentGroup(fulfillmentGroup, fulfillmentGroup.getFulfillmentOption())) {
      ...
      fulfillmentGroup.setSaleShippingPrice(response.getFulfillmentOptionPrices().get(
          fulfillmentGroup.getFulfillmentOption()));
      fulfillmentGroup.setRetailShippingPrice(response.getFulfillmentOptionPrices().get(
          fulfillmentGroup.getFulfillmentOption()));
      fulfillmentGroup.setShippingPrice(response.getFulfillmentOptionPrices().get(
          fulfillmentGroup.getFulfillmentOption()));
  ...
  } // end method calculateCostForFulfillmentGroup
  ...
  @Override public FulfillmentEstimationResponse estimateCostForFulfillmentGroup(FulfillmentGroup fulfillmentGroup,
    ...
    for (FulfillmentOption option : options) {
      if (canCalculateCostForFulfillmentGroup(fulfillmentGroup, option)) {
        ...
        for (FulfillmentGroupItem fulfillmentGroupItem : fulfillmentGroup.getFulfillmentGroupItems()) {
          ...
          if (addToTotal) {
            ...
            BigDecimal price = (fulfillmentGroupItem.getTotalItemAmount() != null)
              ? fulfillmentGroupItem.getTotalItemAmount().getAmount() : null;

            if (price == null) {
              price = fulfillmentGroupItem.getOrderItem().getAveragePrice().getAmount().multiply(BigDecimal.valueOf(
            ...
            if ((sku != null) && (sku.getWeight() != null) && (sku.getWeight().getWeight() != null)) {
              BigDecimal convertedWeight = convertWeight(sku.getWeight().getWeight(),
                  sku.getWeight().getWeightUnitOfMeasure()).multiply(BigDecimal.valueOf(
                    fulfillmentGroupItem.getQuantity()));
        ...
        shippingPrices.put(option,
          BroadleafCurrencyUtils.getMoney(lowestBandFulfillmentPrice, fulfillmentGroup.getOrder().getCurrency()));
      } // end if
    } // end for
   ...
   *
   * @return  default implementation is to convert everything to pounds for consistent weight types.
   */
...
} // end class BandedFulfillmentPricingProvider
