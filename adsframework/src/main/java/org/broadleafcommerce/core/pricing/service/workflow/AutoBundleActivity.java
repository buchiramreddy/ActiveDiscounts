 ...
 *
 * <p>For example, if a ProductBundle exists of two items and the user has one of the items in their cart. If they then
 * add the second item, this activity will replace the two items with the ProductBundle.</p>
 *
 * <p>This only occurs if the ProductBundle is set to "automatically" bundle.</p>
 *
 * @author   $author$
...
public class AutoBundleActivity extends BaseActivity<PricingContext> {
  ...
  public Order handleAutomaticBundling(Order order) throws PricingException, RemoveFromCartException {
    ...
    return order;
  } // end method handleAutomaticBundling
   ...
   * @param   order  DOCUMENT ME!
   *
   * @return  removes all automatic bundles from the order and replaces with DiscreteOrderItems.
   *
  ...
  } // end method unBundleItems

  /**
   * Builds a BundleOrderItem based on the passed in productBundle. Creates new DiscreteOrderItems. Removes the existing
   * matching DiscreteOrderItems or modifies the quantity if needed.
   *
   * @param   order            DOCUMENT ME!
   * @param   productBundle    DOCUMENT ME!
   * @param   numApplications  DOCUMENT ME!
   * @param   unbundledItems   DOCUMENT ME!
   *
   * @return  builds a BundleOrderItem based on the passed in productBundle.
   *
   * @throws  org.broadleafcommerce.core.pricing.service.exception.PricingException
   ...
   */
  private Order bundleItems(Order order, ProductBundle productBundle, Integer numApplications,
    List<DiscreteOrderItem> unbundledItems) throws PricingException, RemoveFromCartException {
    ...
    for (SkuBundleItem skuBundleItem : productBundle.getSkuBundleItems()) {
      ...
      int                     skuMatches   = populateItemMatchesForSku(itemMatches, order, unbundledItems,
          skuBundleItem.getSku().getId());
      int                     skusRequired = skuBundleItem.getQuantity() * numApplications;

      if (skuMatches < skusRequired) {
        throw new IllegalArgumentException(
          "Something went wrong creating automatic bundles.  Not enough skus to fulfill bundle requirements for sku id: "
          + skuBundleItem.getSku().getId());
      }
   ...
   * DOCUMENT ME!
   *
   * @param   matchingItems   DOCUMENT ME!
   * @param   order           DOCUMENT ME!
   * @param   unbundledItems  DOCUMENT ME!
   ...
   * @return  DOCUMENT ME!
   */
  protected int populateItemMatchesForSku(List<DiscreteOrderItem> matchingItems, Order order,
    List<DiscreteOrderItem> unbundledItems, Long skuId) {
    int skuMatches = 0;
   ...
   * @param   processedBundles  DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   */
  ...
  } // end method countMaximumApplications

} // end class AutoBundleActivity
