...
public class ItemOfferProcessorImpl extends OrderOfferProcessorImpl implements ItemOfferProcessor {
   ...
   */
  @Override public void filterItemLevelOffer(PromotableOrder order,
    List<PromotableCandidateItemOffer> qualifiedItemOffers, Offer offer) {
    boolean isNewFormat            = !CollectionUtils.isEmpty(offer.getQualifyingItemCriteria())
      || !CollectionUtils.isEmpty(offer.getTargetItemCriteria());
  ...
  } // end method filterItemLevelOffer
   ...
   */
  protected PromotableCandidateItemOffer createCandidateItemOffer(
  ...
  protected boolean offerMeetsSubtotalRequirements(PromotableOrder order, PromotableCandidateItemOffer itemOffer) {
    if ((itemOffer.getOffer().getQualifyingItemSubTotal() == null)
          || itemOffer.getOffer().getQualifyingItemSubTotal().lessThanOrEqual(Money.ZERO)) {
      return true;
    }
  ...
  /**
   * Private method used by applyAdjustments to create an OrderItemAdjustment from a CandidateOrderOffer and associates
   * the OrderItemAdjustment to the OrderItem.
   *
   * @param  itemOffer        orderOffer a CandidateOrderOffer to apply to an Order
   * @param  itemPriceDetail  DOCUMENT ME!
   */
  ...
  /**
   * The itemOffer has been qualified and prior methods added PromotionDiscount objects onto the ItemPriceDetail. This
   * code will convert the PromotionDiscounts into Adjustments
   *
   * @param  order      DOCUMENT ME!
   * @param  itemOffer  DOCUMENT ME!
   */
   ...
   *
   * @return  the adjustment might not be better than the sale price.
   */
   ...
   * @param   itemOffer  DOCUMENT ME!
   *
   * @return  return false if a totalitarian offer has already been applied and this order already has item adjustments.
   */
  ...
  /**
   * Some promotions can only apply to the retail price. This method determines whether retailPrice only promotions
   * should be used instead of those that can apply to the sale price as well.
   *
   * @param  order  DOCUMENT ME!
   */
   ...
   * @param   order      DOCUMENT ME!
   *
   * @return  loop through ItemCriteria and mark qualifiers required to give the promotion to 1 or more targets.
   */
   ...
   * @param   order      DOCUMENT ME!
   *
   * @return  loop through ItemCriteria and mark targets that can get this promotion to give the promotion to 1 or more
   *          targets.
   */
      ...
      if (receiveQtyNeeded > 0) {
        ...
        if (itemQtyAvailableToBeUsedAsTarget > 0) {
          if ((promotion.getMaxUses() == 0) || (itemOffer.getUses() < promotion.getMaxUses())) {
            ...
            priceDetail.addPromotionDiscount(itemOffer, itemOffer.getOffer().getTargetItemCriteria(),
              qtyToMarkAsTarget);
          }
        }
      }
  ...
  } // end method markTargets

  /**
   * Used in
   * {@link #applyItemQualifiersAndTargets(org.broadleafcommerce.core.offer.service.discount.domain.PromotableCandidateItemOffer, org.broadleafcommerce.core.offer.service.discount.domain.PromotableOrder)}
   ...
   * @return  used in
   *          {@link #applyItemQualifiersAndTargets(org.broadleafcommerce.core.offer.service.discount.domain.PromotableCandidateItemOffer, org.broadleafcommerce.core.offer.service.discount.domain.PromotableOrder)}
   *          allow for customized sorting for which qualifier items should be attempted to be used first for a
   *          promotion.
   */
   ...
   * <p>Used in
   * {@link #applyItemQualifiersAndTargets(org.broadleafcommerce.core.offer.service.discount.domain.PromotableCandidateItemOffer, org.broadleafcommerce.core.offer.service.discount.domain.PromotableOrder)}
   * allow for customized sorting for which target items the promotion should be attempted to be applied to first.
   * Default behavior is to sort descending, so higher-value items get the promotion over lesser-valued items.</p>
   *
   * <p>Note: By default, both the {@link #getQualifierItemComparator(boolean)} and this target comparator are sorted in
   * descending order. This means that higher-valued items can be paired with higher-valued items and lower-valued items
   * can be paired with lower-valued items. This also ensures that you will <b>not</b> have the scenario where 2
   * lower-valued items can be used to qualify a higher-valued target.</p>
   *
   * @param   applyToSalePrice  - whether or not the Comparator should use the sale price for comparison
   *
   * @return  used in
   *          {@link #applyItemQualifiersAndTargets(org.broadleafcommerce.core.offer.service.discount.domain.PromotableCandidateItemOffer, org.broadleafcommerce.core.offer.service.discount.domain.PromotableOrder)}
   *          allow for customized sorting for which target items the promotion should be attempted to be applied to
   *          first.
   */
  ...
  /**
   * @see  org.broadleafcommerce.core.offer.service.processor.ItemOfferProcessor#filterOffers(org.broadleafcommerce.core.offer.service.discount.domain.PromotableOrder,
   *       java.util.List, java.util.List, java.util.List)
   */
  @Override public void filterOffers(PromotableOrder order, List<Offer> filteredOffers,
    List<PromotableCandidateOrderOffer> qualifiedOrderOffers, List<PromotableCandidateItemOffer> qualifiedItemOffers) {
  ...
  } // end method calculatePotentialSavings
  ...
  } // end method markQualifiersAndTargets
  ...
  /**
   * This method could be overridden to potentially run all permutations of offers. A reasonable alternative is to have
   * a permutation with nonCombinable offers and another with combinable offers.
   *
   * @param   offers  DOCUMENT ME!
   *
   * @return  this method could be overridden to potentially run all permutations of offers.
   */
  ...
  } // end method buildItemOfferPermutations
  ...
  } // end method determineBestPermutation

  /**
   * @see  org.broadleafcommerce.core.offer.service.processor.ItemOfferProcessor#applyAndCompareOrderAndItemOffers(org.broadleafcommerce.core.offer.service.discount.domain.PromotableOrder,
...
} // end class ItemOfferProcessorImpl
