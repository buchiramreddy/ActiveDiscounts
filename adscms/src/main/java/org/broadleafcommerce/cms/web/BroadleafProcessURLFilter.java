 ...
 * DOCUMENT ME!
 *
 * @deprecated  In favor of org.broadleafcommerce.common.web.BroadleafRequestFilter. formally component name
 *              "blProcessURLFilter"
 *
 *              <p>This filter sets up the CMS system by setting the current sandbox, locale, time of day, and
 *              languageCode that used by content items.
 *              </p>After setting up content variables, it checks to see if a request can be processed by an instance of
 *              URLProcessor and if so, delegates the request to that processor.
 *
 *              <p>This filter creates an internal cache to quickly determine if the request should be processed by an
 *              instance of URLProcessor or be passed to the next filter in the filter chain. The cache settings
 *              (including expiration seconds, maximum elements, and concurrency) can be configured via Spring at
 *              startup. See {@code com.google.common.cache.CacheBuilder} for more information on these parameters.</p>
 * @author      bpolster
...
public class BroadleafProcessURLFilter extends OncePerRequestFilter {
  ...
  private static final SimpleDateFormat CONTENT_DATE_PARSE_FORMAT               = new SimpleDateFormat(
      "MM/dd/yyyy hh:mm aa");
   ...
   * DOCUMENT ME!
   *
   * @param  cacheExpirationSeconds  DOCUMENT ME!
   */
   ...
   * DOCUMENT ME!
   *
   * @param  maxCacheConcurrency  DOCUMENT ME!
   */
   ...
   * DOCUMENT ME!
   *
   * @param  maxCacheElements  DOCUMENT ME!
   */
   ...
   * DOCUMENT ME!
   *
   * @param  sandBoxPreviewEnabled  DOCUMENT ME!
   */
   ...
   * DOCUMENT ME!
   *
   * @param  urlProcessorList  DOCUMENT ME!
   */
  ...
  protected boolean shouldProcessURL(HttpServletRequest request, String requestURI) {
    ...
    return true;
  } // end method shouldProcessURL

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * If another filter has already set the language as a request attribute, that will be honored. Otherwise, the request
   * parameter is checked followed by the session attribute.
   *
   * @param   request  DOCUMENT ME!
   * @param   site     DOCUMENT ME!
   *
   * @return  if another filter has already set the language as a request attribute, that will be honored.
   */
   ...
   *
   * @return  true if the passed in sandbox is null or is of type SandBoxType.PRODUCTION.
   */
   ...
   *
   * @param   requestURIWithoutContextPath  DOCUMENT ME!
   *
   * @return  builds a cache for each URL that determines how it should be processed.
   *
  ...
  private URLProcessor lookupProcessorFromCache(String requestURIWithoutContextPath) throws ExecutionException {
    if (urlCache == null) {
      urlCache = CacheBuilder.newBuilder().maximumSize(maxCacheElements).concurrencyLevel(maxCacheConcurrency)
        .expireAfterWrite(cacheExpirationSeconds, TimeUnit.SECONDS).build(new CacheLoader<String, URLProcessor>() {
  ...
  } // end method setContentTime
} // end class BroadleafProcessURLFilter
