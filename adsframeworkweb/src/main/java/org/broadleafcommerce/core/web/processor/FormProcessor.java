...
public class FormProcessor extends AbstractElementProcessor {
  ...
  /**
   * We need this replacement to execute as early as possible to allow subsequent processors to act on this element as
   * if it were a normal form instead of a blc:form.
   *
   * @return  we need this replacement to execute as early as possible to allow subsequent processors to act on this
   *          element as if it were a normal form instead of a blc:form.
   */
    ...
    if (!"GET".equalsIgnoreCase(element.getAttributeValueFromNormalizedName("method"))) {
      try {
        ...
        if ("multipart/form-data".equalsIgnoreCase(element.getAttributeValueFromNormalizedName("enctype"))) {
          String action             = (String) StandardExpressionProcessor.processExpression(arguments,
              element.getAttributeValueFromNormalizedName("th:action"));
