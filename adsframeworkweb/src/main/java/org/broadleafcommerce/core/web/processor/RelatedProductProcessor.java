...
/**
 * A Thymeleaf processor that will find related products. A product or category id must be specified. If both are
 * specified, only the productId will be used.
 *
 * <p>Takes in the following parameters</p>
 *
 * <ul>
 *   <li>productId - productId to find related products.</li>
 *   <li>categoryId - categoryId to find related products.</li>
 *   <li>type - the type of relations to find (e.g. FEATURED (DEFAULT), UPSELL, CROSSSELL). Implementations may have
 *     other specific types of related products.</li>
 *   <li>cumulativeResults - true (DEFAULT) /false - indicates that the system should add results from the parent
 *     categories of the passed in item as well as the current item</li>
 *   <li>qty - if specified, determines the max-number of results that will be returned; otherwise, all results are
 *     returned.</li>
 *   <li>productsResultVar - if specified, adds the products to the model keyed by this var. Otherwise, uses "products"
 *     as the model identifier.</li>
 *   <li>relatedProductsResultVar - if specified, adds the RelatedProduct(s) to the model keyed by this var. Otherwise,
 *     uses "relatedProducts" as the model identifier.</li>
 * </ul>
 *
 * <p>The output from this operation returns a list of PromotableProducts which represent the following.
 * relatedProduct.product relatedProduct.promotionMessage.</p>
 *
 * @author   bpolster
...
public class RelatedProductProcessor extends AbstractModelVariableModifierProcessor {
   ...
   * DOCUMENT ME!
   *
   * @param   relatedProducts  DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   */
  ...
  protected void modifyModelAttributes(Arguments arguments, Element element) {
    List<? extends PromotableProduct> relatedProducts = relatedProductsService.findRelatedProducts(buildDTO(arguments,
          element));
  ...
  private RelatedProductDTO buildDTO(Arguments args, Element element) {
    ...
    if (quantityStr != null) {
      relatedProductDTO.setQuantity(((BigDecimal) StandardExpressionProcessor.processExpression(args, quantityStr))
        .intValue());
    }
...
} // end class RelatedProductProcessor
