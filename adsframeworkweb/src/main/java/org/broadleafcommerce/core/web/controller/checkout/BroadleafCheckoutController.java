...
public class BroadleafCheckoutController extends AbstractCheckoutController {
   ...
   *
   * <p>If the paymentMethod is undefined or "creditCard" delegates to the "completeSecureCreditCardCheckout" method.
   * </p>
   *
   * <p>Otherwise, returns an operation not supported.</p>
   *
   * <p>This method assumes that a credit card payment info will be either sent to a third party gateway or saved in a
   * secure schema. If the transaction is successful, the order will be assigned an order number, its status change to
   * SUBMITTED, and given a submit date. The method then returns the default confirmation path
   * "/confirmation/{orderNumber}"</p>
   *
   * <p>If the transaction is unsuccessful, (e.g. the gateway declines payment) processFailedOrderCheckout() is called
   * and reverses the state of the order.</p>
   *
   * <p>Note: this method removes any existing payment infos of type CREDIT_CARD and re-creates it with the information
   * from the BillingInfoForm</p>
   *
   * @param   request      DOCUMENT ME!
   * @param   response     DOCUMENT ME!
   * @param   model        DOCUMENT ME!
  ...
  public String completeCheckout(HttpServletRequest request, HttpServletResponse response, Model model,
    ...
    if ((billingForm.getPaymentMethod() == null) || "credit_card".equals(billingForm.getPaymentMethod())) {
      ...
      throw new IllegalArgumentException("Complete checkout called with payment Method "
  ...
  public String completeSecureCreditCardCheckout(HttpServletRequest request, HttpServletResponse response, Model model,
    ...
    if (cart != null) {
      ...
      CreditCardPaymentInfo ccReference = (CreditCardPaymentInfo) securePaymentInfoService.create(
  ...
  } // end method completeSecureCreditCardCheckout
   ...
   * @param   request        DOCUMENT ME!
   * @param   model          DOCUMENT ME!
   * @param   orderInfoForm  errors
   * @param   result         emailAddress
   *
   * @return  the return path
   *
   * @throws  org.broadleafcommerce.common.exception.ServiceException
  ...
  public String saveGlobalOrderDetails(HttpServletRequest request, Model model,
    ...
    return getCheckoutPageRedirect();
  } // end method saveGlobalOrderDetails
   ...
   *
   * @see     #showMultiship(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse,
  ...
  /**
   * Processes the requested add address from the multiship process. This method will create a CustomerAddress based on
   * the requested Address and associate it with the current Customer in session.
   *
   * @param   request      DOCUMENT ME!
   * @param   response     DOCUMENT ME!
   * @param   model        DOCUMENT ME!
   ...
   * @param   response         DOCUMENT ME!
   * @param   model            DOCUMENT ME!
   * @param   instructionForm  DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   *
   ...
   * @throws  PricingException  DOCUMENT ME!
   */
  public String savePaymentForm(HttpServletRequest request, HttpServletResponse response, Model model)
    throws PricingException {
    // TODO: Implement
   ...
   *
   * <p>Note that the default Broadleaf implementation creates an order with a single fulfillment group. In the case of
   * shipping to mutiple addresses, the multiship methods should be used.</p>
   *
   * @param   request       DOCUMENT ME!
   * @param   response      DOCUMENT ME!
   * @param   model         DOCUMENT ME!
  ...
  public String saveSingleShip(HttpServletRequest request, HttpServletResponse response, Model model,
    ...
    return isAjaxRequest(request) ? getCheckoutView() : getCheckoutPageRedirect();
  } // end method saveSingleShip

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * Renders the multiship page. This page is used by the user when shipping items to different locations (or with
   * different FulfillmentOptions) is desired.
   *
   * <p>Note that the default Broadleaf implementation will require the user to input an Address and FulfillmentOption
   * for each quantity of each DiscreteOrderItem.</p>
   *
   * @param   request   DOCUMENT ME!
   * @param   response  DOCUMENT ME!
   * @param   model     DOCUMENT ME!
  ...
  /**
   * This method will copy the shipping address of the first fulfillment group on the order to the billing address on
   * the BillingInfoForm that is passed in.
   *
   * @param  order            DOCUMENT ME!
   * @param  billingInfoForm  DOCUMENT ME!
   */
  ...
  /**
   * Initializes some custom binding operations for the checkout flow. More specifically, this method will attempt to
   * bind state and country abbreviations to actual State and Country objects when the String representation of the
  ...
  protected void populateModelWithShippingReferenceData(HttpServletRequest request, Model model) {
    ...
    model.addAttribute("expirationMonths", populateExpirationMonths());
    model.addAttribute("expirationYears", populateExpirationYears());
  } // end method populateModelWithShippingReferenceData
  ...
  protected void putFulfillmentOptionsAndEstimationOnModel(Model model) {
    ...
    if (!(cart instanceof NullOrderImpl) && (cart.getFulfillmentGroups().size() > 0)
      ...
      try {
        estimateResponse = fulfillmentPricingService.estimateCostForFulfillmentGroup(cart.getFulfillmentGroups().get(0),
            options);
...
} // end class BroadleafCheckoutController
