...
public class MVELTest extends BaseTest {
  ...
  /**
   * Creates a new MVELTest object.
   *
   * @throws  RuntimeException  DOCUMENT ME!
   */
  public MVELTest() {
    InputStream is = this.getClass().getResourceAsStream(
        "/org/broadleafcommerce/core/offer/service/mvelFunctions.mvel");
   ...
   * DOCUMENT ME!
   */
  @Test public void testBasicMVELFunctions() {
    ...
    StringBuffer            functions   = new StringBuffer(
        "def any(x, y) { return x or y } def all(x, y) { return x and y } ");
...
// }
   ...
   * DOCUMENT ME!
   */
  @Test public void testOfferAppliesToFulfillmentGroup() {
    OrderImpl order = new OrderImpl();
    ...
    Serializable domainExp1 = MVEL.compileExpression(
        "offer.type.equals(OfferType.FULFILLMENT_GROUP) and (($ in order.fulfillmentGroups if $.type.equals(FulfillmentType.PHYSICAL)) != empty)",
        context);
  ...
  } // end method testOfferAppliesToFulfillmentGroup
  ...
  public void testOfferAppliesToHatsWhenOneLawnmowerIsPurchased() {
    ...
    // This test makes use of the static MVEL function "orderContains(quantity)".
    StringBuffer expression = new StringBuffer(functions);
    expression.append("def evalItemForOrderContains(item) {").append(
      "             return item.product.defaultCategory.name == 'lawnmower'").append("          } ").append(
      "          return (orderContainsPlusMark(1) and currentItem.product.defaultCategory.name == 'hat');");

    Boolean result = (Boolean) MVEL.eval(expression.toString(), vars);
    assert (result != null) && result;
  } // end method testOfferAppliesToHatsWhenOneLawnmowerIsPurchased
   ...
   * DOCUMENT ME!
   */
  @Test public void testOfferAppliesToItemsInCategoryAndOrderValueGreaterThanFifty() {
    // ----------------------------------------------------------------------------------------------------
    ...
    // Compile the MVEL Expression
    Serializable domainExp1 = MVEL.compileExpression(
        "result = false; for (cat : currentItem.product.allParentCategories) {if (cat.name == 't-shirt') {result = true;}}; return result and order.subTotal.amount >= 50",
        context);
    ...
    Serializable domainExp2         = MVEL.compileExpression(
        "($ in currentItem.product.allParentCategories if $.name == 't-shirt') != empty and order.subTotal.amount >= 50",
        context);
  ...
  } // end method testOfferAppliesToItemsInCategoryAndOrderValueGreaterThanFifty
   ...
   * DOCUMENT ME!
   */
  @Test public void testOfferAppliesToSpecificItems() {
    DiscreteOrderItemImpl orderItem = new DiscreteOrderItemImpl();
    Sku                   sku       = new SkuImpl();
    ...
    // Compile the MVEL Expression
    Serializable domainExp1 = MVEL.compileExpression(
        "offer.type == OfferType.ORDER_ITEM and (currentItem.sku.id in [1234, 2345, 5678])", context);
  ...
  } // end method testOfferAppliesToSpecificItems
} // end class MVELTest
