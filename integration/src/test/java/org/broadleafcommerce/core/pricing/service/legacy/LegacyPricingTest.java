...
public class LegacyPricingTest extends BaseTest {
   ...
   */
  @Test(
    groups          = { "testPricingLegacy" },
    dependsOnGroups = { "testShippingInsertLegacy", "createCustomerIdGenerationLegacy" }
  )
  @Transactional public void testPricing() throws Exception {
    ...
    order.addOfferCode(createOfferCode("20 Percent Off Item Offer", OfferType.ORDER_ITEM, OfferDiscountType.PERCENT_OFF,
        20, null, "discreteOrderItem.sku.name==\"Test Sku\""));
    order.addOfferCode(createOfferCode("3 Dollars Off Item Offer", OfferType.ORDER_ITEM, OfferDiscountType.AMOUNT_OFF,
        3, null, "discreteOrderItem.sku.name!=\"Test Sku\""));
    order.addOfferCode(createOfferCode("1.20 Dollars Off Order Offer", OfferType.ORDER, OfferDiscountType.AMOUNT_OFF,
        1.20, null, null));
    ...
    assert (order.getTotalTax().equals(
          (order.getSubTotal().subtract(order.getOrderAdjustmentsValue())).multiply(0.05D))); // Shipping is not taxable
    assert (order.getTotal().equals(
          order.getSubTotal().add(order.getTotalTax()).add(order.getTotalShipping()).subtract(
            order.getOrderAdjustmentsValue())));
  } // end method testPricing
  ...
  @Rollback(false)
  @Test(
    groups            = { "testShippingInsertLegacy" },
  ...
  private Offer createOffer(String offerName, OfferType offerType, OfferDiscountType discountType, double value,
    String customerRule, String orderRule) {
  ...
  private OfferCode createOfferCode(String offerName, OfferType offerType, OfferDiscountType discountType, double value,
    String customerRule, String orderRule) {
