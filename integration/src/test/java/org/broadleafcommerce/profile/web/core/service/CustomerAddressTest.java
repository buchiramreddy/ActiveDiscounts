...
public class CustomerAddressTest extends CommonSetupBaseTest {
  ...
  /**
   * This method only exists because so many other tests depend on it, but should be removed once tests are more
   * isolated.
   *
  ...
  @Rollback(false)
  @Test(
    groups            = "createCustomerAddress",
    dataProvider      = "setupCustomerAddress",
    dataProviderClass = CustomerAddressDataProvider.class,
    dependsOnGroups   = { "readCustomer", "createCountry", "createState" }
  )
  @Transactional public void createCustomerAddress(CustomerAddress customerAddress) {
    userName = "customer1";
  ...
  @Transactional public void createNewDefaultAddress() {
    ...
    List<CustomerAddress> customerAddressList = customerAddressService.readActiveCustomerAddressesByCustomerId(
        customer.getId());
  ...
  } // end method createNewDefaultAddress

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * TThis method only exists because so many other tests depend on it, but should be removed once tests are more
   * isolated.
   */
  @Deprecated
  @Test(
    groups          = "readCustomerAddress",
    dependsOnGroups = "createCustomerAddress"
  )
  @Transactional public void readCustomerAddressByUserId() {
