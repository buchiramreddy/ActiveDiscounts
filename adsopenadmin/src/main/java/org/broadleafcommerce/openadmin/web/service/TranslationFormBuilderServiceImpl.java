...
public class TranslationFormBuilderServiceImpl implements TranslationFormBuilderService {
  ...
  /**
   * @see  org.broadleafcommerce.openadmin.web.service.TranslationFormBuilderService#buildListGrid(java.util.List, boolean)
   */
  @Override public ListGrid buildListGrid(List<Translation> translations, boolean isRte) {
    ...
    headerFields.add(new Field().withName("translatedValue").withFriendlyName("Translation_translatedValue").withOrder(
        10));
    ...
    for (Translation t : translations) {
      ...
      record.getFields().add(new Field().withName("localeCode").withFriendlyName("Translation_localeCode").withOrder(
          0).withValue(locale.getLocaleCode()).withDisplayValue(locale.getFriendlyName()));

      record.getFields().add(new Field().withName("translatedValue").withFriendlyName("Translation_translatedValue")
        .withOrder(10).withValue(t.getTranslatedValue()).withDisplayValue(
          isRte ? getLocalizedEditToViewMessage() : t.getTranslatedValue()));

      listGrid.getRecords().add(record);
    }

    return listGrid;
  } // end method buildListGrid

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * @see  org.broadleafcommerce.openadmin.web.service.TranslationFormBuilderService#buildTranslationForm(org.broadleafcommerce.openadmin.web.form.TranslationForm)
   */
  @Override public EntityForm buildTranslationForm(TranslationForm formProperties) {
    ...
    ef.addField(new Field().withName("translatedValue").withFieldType(formProperties.getIsRte() ? "html" : "string")
      .withFriendlyName("Translation_translatedValue").withValue(formProperties.getTranslatedValue()).withOrder(10));
...
} // end class TranslationFormBuilderServiceImpl
