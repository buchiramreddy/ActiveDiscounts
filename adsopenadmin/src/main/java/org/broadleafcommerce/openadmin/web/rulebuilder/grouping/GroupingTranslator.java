...
public class GroupingTranslator {
  ...
  public Group createGroups(String mvel) throws MVELTranslationException {
    ...
    if (tokens.length > 1) {
      throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_RULE,
        "mvel expressions must resolve to a boolean result. "
  ...
  protected void compilePhrases(String segment, Group myGroup, boolean isNegation) throws MVELTranslationException {
    ...
    if ((andTokens.length > 1) && (orTokens.length > 1)) {
      throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_RULE,
        "Segments that mix logical operators are not compatible with "
   ...
   *
   * @param   segment             DOCUMENT ME!
   * @param   subgroupStartIndex  DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   *
   * @throws  MVELTranslationException  DOCUMENT ME!
  ...
  protected int findGroupEnd(String segment, int subgroupStartIndex) throws MVELTranslationException {
    ...
    throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_RULE,
      "Unable to find an end parenthesis for the group started at ("
   ...
   * @param   segment  DOCUMENT ME!
   * @param   myGroup  DOCUMENT ME!
   * @param   isAnd    DOCUMENT ME!
   * @param   isOr     DOCUMENT ME!
   * @param   isNot    DOCUMENT ME!
   *
   * @throws  MVELTranslationException  DOCUMENT ME!
   */
  protected void setGroupOperator(String segment, Group myGroup, boolean isAnd, boolean isOr, boolean isNot)
    ...
    if (myGroup.getOperatorType() == null) {
      ...
      if ((isOr && !myGroup.getOperatorType().toString().equals(BLCOperator.OR.toString()))) {
        throw new MVELTranslationException(MVELTranslationException.INCOMPATIBLE_RULE,
          "Segment logical operator is not compatible with the group "
