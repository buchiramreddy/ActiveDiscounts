...
public class AdminBasicEntityController extends AdminAbstractController {
   ...
   * @param   collectionField  DOCUMENT ME!
  ...
  /**
   * Invoked on every request to provide the ability to register specific binders for Spring's binding process. By
   * default, we register a binder that treats empty Strings as null and a Boolean editor that supports either true or
   * false. If the value is passed in as null, it will treat it as false.
   *
   ...
   *
   * <p>Note that the request must contain a parameter called "key" when attempting to remove a collection item from a
   * map collection.</p>
   *
   * @param   request           DOCUMENT ME!
   * @param   response          DOCUMENT ME!
   ...
   */
  @RequestMapping(
    value  = "/{id}/{collectionField:.*}/{collectionItemId}/delete",
    method = RequestMethod.POST
  )
  ...
  /**
   * Attempts to save the given entity. If validation is unsuccessful, it will re-render the entity form with error
   * fields highlighted. On a successful save, it will refresh the entity page.
   *
   * @param   request     DOCUMENT ME!
   * @param   response    DOCUMENT ME!
   * @param   model       DOCUMENT ME!
  ...
  } // end method saveEntity

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * Shows the modal dialog that is used to add an item to a given collection. There are several possible outcomes of
   * this call depending on the type of the specified collection field.
   *
   * <ul>
   *   <li><b>Basic Collection (Persist)</b> - Renders a blank form for the specified target entity so that the user may
   *     enter information and associate the record with this collection. Used by fields such as ProductAttribute.</li>
   *   <li><b>Basic Collection (Lookup)</b> - Renders a list grid that allows the user to click on an entity and select
   *     it. Used by fields such as "allParentCategories".</li>
   *   <li><b>Adorned Collection (without form)</b> - Renders a list grid that allows the user to click on an entity and
   *     select it. The view rendered by this is identical to basic collection (lookup), but will perform the operation
   *     on an adorned field, which may carry extra meta-information about the created relationship, such as order.</li>
   *   <li><b>Adorned Collection (with form)</b> - Renders a list grid that allows the user to click on an entity and
   ...
   *     to linking an entity, provide extra fields, such as a promotional message.</li>
   *   <li><b>Map Collection</b> - Renders a form for the target entity that has an additional key field. This field is
   *     populated either from the configured map keys, or as a result of a lookup in the case of a key based on another
   *     entity. Used by fields such as the mediaMap on a Sku.</li>
   * </ul>
   *
   * @param   request          DOCUMENT ME!
   * @param   response         DOCUMENT ME!
   * @param   model            DOCUMENT ME!
   * @param   pathVars         sectionKey
   * @param   id               DOCUMENT ME!
   * @param   collectionField  DOCUMENT ME!
   * @param   requestParams    DOCUMENT ME!
   *
  ...
  public String showAddCollectionItem(HttpServletRequest request, HttpServletResponse response, Model model,
    ...
    PersistencePackageRequest ppr = PersistencePackageRequest.fromMetadata(md).withFilterAndSortCriteria(getCriteria(
          requestParams)).withStartIndex(getStartIndex(requestParams)).withMaxIndex(getMaxIndex(requestParams));
  ...
  /**
   * Shows the modal dialog that is used to select a "to-one" collection item. For example, this could be used to show a
   * list of categories for the ManyToOne field "defaultCategory" in Product.
   *
   * @param   request          DOCUMENT ME!
   * @param   response         DOCUMENT ME!
   * @param   model            DOCUMENT ME!
   ...
   */
  @RequestMapping(
    value  = "/{owningClass:.*}/{collectionField:.*}/select",
    method = RequestMethod.GET
  )
  public String showSelectCollectionItem(HttpServletRequest request, HttpServletResponse response, Model model,
    ...
    return "modules/modalContainer";
  } // end method showSelectCollectionItem
   ...
   */
  @RequestMapping(
    value  = "/{id}/{collectionField:.*}/{collectionItemId}/view",
    method = RequestMethod.GET
  )
  ...
  public String updateCollectionItem(HttpServletRequest request, HttpServletResponse response, Model model,
    ...
    // First, we must save the collection entity
    Entity savedEntity = service.updateSubCollectionEntity(entityForm, mainMetadata, collectionProperty, entity,
        collectionItemId);
    entityFormValidator.validate(entityForm, savedEntity, result);
    ...
    return "views/standaloneListGrid";
  } // end method updateCollectionItem

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * Updates the given colleciton item's sequence. This should only be triggered for adorned target collections where a
   * sort field is specified -- any other invocation is incorrect and will result in an exception.
   *
   * @param   request           DOCUMENT ME!
   * @param   response          DOCUMENT ME!
   * @param   model             DOCUMENT ME!
   ...
   */
  @RequestMapping(
    value  = "/{id}/{collectionField:.*}/{collectionItemId}/sequence",
    method = RequestMethod.POST
  )
  @ResponseBody public Map<String, Object> updateCollectionItemSequence(HttpServletRequest request,
  ...
  } // end method updateCollectionItemSequence

  //~ ------------------------------------------------------------------------------------------------------------------

  /**
   * Renders the modal form that is used to add a new parent level entity. Note that this form cannot render any
   * subcollections as operations on those collections require the parent level entity to first be saved and have and
   * id. Once the entity is initially saved, we will redirect the user to the normal manage entity screen where they can
   * then perform operations on sub collections.
   *
   * @param   request     DOCUMENT ME!
   * @param   response    DOCUMENT ME!
   * @param   model       DOCUMENT ME!
   ...
   * @param   id               DOCUMENT ME!
   *
   * @return  shows the modal popup for the current selected "to-one" field.
   *
   * @throws  Exception
   */
  @RequestMapping(
    value  = "/{collectionField:.*}/{id}/view",
    method = RequestMethod.GET
  )
  ...
  /**
   * Renders the main entity listing for the specified class, which is based on the current sectionKey with some
   * optional criteria.
   *
   * @param   request        DOCUMENT ME!
   * @param   response       DOCUMENT ME!
   * @param   model          DOCUMENT ME!
   * @param   pathVars       DOCUMENT ME!
   * @param   requestParams  criteria a Map of property name -> list critiera values
   *
   * @return  the return view path
   *
   * @throws  Exception
...
checkReadOnly:  {
  ...
  /**
   * Builds out all of the model information needed for showing the add modal for collection items on both the initial
   * GET as well as after a POST with validation errors.
   *
   * @param   request             DOCUMENT ME!
   * @param   response            DOCUMENT ME!
   * @param   model               DOCUMENT ME!
   ...
   * @see
   *          
   *          {@link #addCollectionItem(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, org.springframework.ui.Model, java.util.Map, String, String, org.broadleafcommerce.openadmin.web.form.entity.EntityForm, org.springframework.validation.BindingResult)}
   * @see
   *          
   *          {@link #showAddCollectionItem(javax.servlet.http.HttpServletRequest, javax.servlet.http.HttpServletResponse, org.springframework.ui.Model, java.util.Map, String, String, org.springframework.util.MultiValueMap)}
   *
  ...
  protected String buildAddCollectionItemModel(HttpServletRequest request, HttpServletResponse response,
    ...
    return "modules/modalContainer";
  } // end method buildAddCollectionItemModel
   ...
   * @param   collectionField   DOCUMENT ME!
   * @param   collectionItemId  DOCUMENT ME!
   * @param   modalHeaderType   DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   *
  ...
  protected String showViewUpdateCollection(HttpServletRequest request, Model model, Map<String, String> pathVars,
    ...
    return showViewUpdateCollection(request, model, pathVars, id, collectionField, collectionItemId, modalHeaderType,
        null, null);
  }
  ...
  /**
   * Shows the view and populates the model for updating a collection item. You can also pass in an entityform and
   * entity which are optional. If they are not passed in then they are automatically looked up
   *
   * @param   request           DOCUMENT ME!
   * @param   model             DOCUMENT ME!
   * @param   pathVars          DOCUMENT ME!
   ...
   * @param   entity            DOCUMENT ME!
   *
   * @return  shows the view and populates the model for updating a collection item.
   *
   * @throws  org.broadleafcommerce.common.exception.ServiceException
   */
  protected String showViewUpdateCollection(HttpServletRequest request, Model model, Map<String, String> pathVars,
    String id, String collectionField, String collectionItemId, String modalHeaderType, EntityForm entityForm,
    ...
    return "modules/modalContainer";
  } // end method showViewUpdateCollection

} // end class AdminBasicEntityController
