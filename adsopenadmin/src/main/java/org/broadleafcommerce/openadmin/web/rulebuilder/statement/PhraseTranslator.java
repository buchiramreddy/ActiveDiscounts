...
public class PhraseTranslator {
  ...
  protected String[] extractComponents(String phrase) throws MVELTranslationException {
    ...
    if (components.length != 3) {
      ...
      if (components.length != 3) {
        ...
        if (components.length != 3) {
          throw new MVELTranslationException(MVELTranslationException.UNRECOGNIZABLE_RULE,
            "Could not parse the MVEL expression to a "
   ...
   * @param   isIgnoreCase       DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   *
   * @throws  MVELTranslationException  DOCUMENT ME!
...
// return BLCOperator.ENDS_WITH;
    ...
    throw new MVELTranslationException(MVELTranslationException.OPERATOR_NOT_FOUND,
      "Unable to identify an operator compatible with the "
      + "rules builder: (" + (isNegation ? "!" : ("" + field + operator + value)) + ")");
  } // end method getOperator

} // end class PhraseTranslator
