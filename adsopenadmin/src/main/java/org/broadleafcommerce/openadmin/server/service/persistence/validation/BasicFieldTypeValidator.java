...
public class BasicFieldTypeValidator implements PopulateValueRequestValidator {
  ...
  /**
   * @see  org.broadleafcommerce.openadmin.server.service.persistence.validation.PopulateValueRequestValidator#validate(org.broadleafcommerce.openadmin.server.service.persistence.module.provider.request.PopulateValueRequest,
  ...
  @Override public PropertyValidationResult validate(PopulateValueRequest populateValueRequest, Serializable instance) {
    switch (populateValueRequest.getMetadata().getFieldType()) {
      ...
      case MONEY: {
        try {
          if (BigDecimal.class.isAssignableFrom(populateValueRequest.getReturnType())
                || Money.class.isAssignableFrom(populateValueRequest.getReturnType())) {
            new BigDecimal(populateValueRequest.getRequestedValue());
      ...
      case DATE: {
        try {
          ...
          return new PropertyValidationResult(false,
              "Field must be a date of the format: "
      ...
      case ADDITIONAL_FOREIGN_KEY: {
        if (Collection.class.isAssignableFrom(populateValueRequest.getReturnType())) {
          ...
          try {
            ...
            return new PropertyValidationResult(false,
                "External entity cannot be added to the specified collection at "
                + populateValueRequest.getProperty().getName());
          ...
          return new PropertyValidationResult(false,
              "External entity cannot be added to a map at " + populateValueRequest.getProperty().getName());
        }
      }
...
} // end class BasicFieldTypeValidator
