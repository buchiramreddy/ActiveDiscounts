...
public class MapStructurePersistenceModule extends BasicPersistenceModule {
  ...
  @Override public Entity add(PersistencePackage persistencePackage) throws ServiceException {
    ...
    MapStructure           mapStructure           = (MapStructure)
      persistencePerspective.getPersistencePerspectiveItems().get(PersistencePerspectiveItemType.MAPSTRUCTURE);
    ...
    try {
      ...
      String                     mapKey                  = entity.findProperty(mapStructure.getKeyPropertyName())
        .getValue();
      ...
      if (ceilingMergedProperties.containsKey(
              mapStructure.getMapProperty() + FieldManager.MAPFIELDSEPARATOR + mapKey)) {
      ...
      Entity[] responses = getMapRecords(instance, mapStructure, ceilingMergedProperties, valueMergedProperties,
          entity.findProperty("symbolicId"));

      for (Entity response : responses) {
        if (response.findProperty(mapStructure.getKeyPropertyName()).getValue().equals(
                persistencePackage.getEntity().findProperty(mapStructure.getKeyPropertyName()).getValue())) {
          return response;
        }
      }
  ...
  @Override public void extractProperties(Class<?>[] inheritanceLine,
    ...
    if (mergedProperties.get(MergedPropertyType.MAPSTRUCTUREVALUE) != null) {
      extractPropertiesFromMetadata(inheritanceLine, mergedProperties.get(MergedPropertyType.MAPSTRUCTUREVALUE),
        properties, false, MergedPropertyType.MAPSTRUCTUREVALUE);
    }
  }
    ...
    try {
      ...
      if (persistentClass != null) {
        ...
        if (StringUtils.isNotEmpty(mapKey) && !entity.isValidationFailure()) {
          map.put(entity.findProperty(mapStructure.getKeyPropertyName()).getValue(),
            entity.findProperty(((SimpleValueMapStructure) mapStructure).getValuePropertyName()).getValue());
        }
      }
      ...
      return
        getMapRecords(instance, mapStructure, ceilingMergedProperties, valueMergedProperties,
          entity.findProperty("symbolicId"))[0];
    } catch (Exception e) {
      throw new ServiceException("Problem updating entity : " + e.getMessage(), e);
    } // end try-catch
  } // end method update
...
// }
   ...
   * @param   valueMergedProperties  DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   *
  ...
  } // end method convertPrimitiveBasedOnType
   ...
   * @param   ceilingMergedProperties  DOCUMENT ME!
   * @param   valueMergedProperties    DOCUMENT ME!
   * @param   symbolicIdProperty       DOCUMENT ME!
   *
   * @return  DOCUMENT ME!
   *
  ...
  protected Entity[] getMapRecords(Serializable record, MapStructure mapStructure,
    ...
    Property symbolicIdProperty) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException,
    SecurityException, IllegalArgumentException, ClassNotFoundException, NoSuchFieldException {
  ...
  } // end method getMapRecords

} // end class MapStructurePersistenceModule
