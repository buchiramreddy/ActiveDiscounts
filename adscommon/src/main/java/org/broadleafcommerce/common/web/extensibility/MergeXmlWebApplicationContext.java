 ...
 * documents, understood by an {@link org.springframework.beans.factory.xml.XmlBeanDefinitionReader}.
 *
 * <p>By default, the configuration will be taken from "/WEB-INF/applicationContext.xml" for the rootId context, and
 * "/WEB-INF/test-servlet.xml" for a context with the namespace "test-servlet" (like for a DispatcherServlet instance
 * with the servlet-name "test").</p>
 *
 * <p>The config location defaults can be overridden via the "contextConfigLocation" context-param of
 ...
 * {@link org.springframework.util.PathMatcher} javadoc for pattern details).</p>
 *
 * <p>Note: In case of multiple config locations, later bean definitions will override ones defined in earlier loaded
 * files. This can be leveraged to deliberately override certain bean definitions via an extra XML file.</p>
 *
 * <p>In addition to standard configuration, this implementation also takes a list of patch configuration files that are
 * merged into the configuration provided above.
 * {@link org.broadleafcommerce.profile.extensibility.MergeXmlConfigResourceFactory}. The patch file locations are set
 * via the "patchConfigLocation" context-param of
 * {@link org.broadleafcommerce.common.web.extensibility.MergeContextLoader}. Patch locations can either denote concrete
 * files like "/WEB-INF/patch.xml" or Ant-style patterns like "/WEB-INF/*-context.xml" (see
 * {@link org.springframework.util.pathMatcher} javadoc for pattern details).</p>
 *
 * @author   Jeff Fischer
...
public class MergeXmlWebApplicationContext extends XmlWebApplicationContext {
   ...
   * supposed to load and/or register bean definitions.</p>
   *
   * <p>Delegates to a ResourcePatternResolver for resolving location patterns into Resource instances.</p>
   *
  ...
  @Override protected void loadBeanDefinitions(XmlBeanDefinitionReader reader) throws BeansException, IOException {
    ...
    for (int i = 0; i < patchLocations.length; i++) {
      if (patchLocations[i].startsWith("classpath")) {
        InputStream is = MergeXmlWebApplicationContext.class.getClassLoader().getResourceAsStream(patchLocations[i]
            .substring("classpath*:".length(), patchLocations[i].length()));
      ...
      if ((patches[i] == null) || (patches[i].available() <= 0)) {
        throw new IOException("Unable to open an input stream on specified application context resource: "
          + patchLocations[i]);
      }
    }
  ...
  } // end method loadBeanDefinitions

} // end class MergeXmlWebApplicationContext
