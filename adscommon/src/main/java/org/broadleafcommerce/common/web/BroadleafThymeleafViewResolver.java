...
/**
 * This class extends the default ThymeleafViewResolver to facilitate rendering template fragments (such as those used
 * by AJAX modals or iFrames) within a full page container should the request for that template have occurred in a
 * stand-alone context.
 *
...
public class BroadleafThymeleafViewResolver extends ThymeleafViewResolver {
  ...
  /**
   * <p>Prefix to be used in view names (returned by controllers) for specifying an HTTP redirect with AJAX support.
   * That is, if you want a redirect to be followed by the browser as the result of an AJAX call or within an iFrame at
   * the parent window, you can utilize this prefix. Note that this requires a JavaScript component, which is provided
   * as part of BLC.js</p>
   *
   * <p>If the request was not performed in an AJAX / iFrame context, this method will delegate to the normal
   * "redirect:" prefix.</p>
   *
   * <p>Value: <tt>ajaxredirect:</tt></p>
   */
   ...
   *
   * @param  fullPageLayout  DOCUMENT ME!
   */
   ...
   * @see    #getIframeLayout()
   *
   * @param  iframeLayout  DOCUMENT ME!
   */
  ...
  protected boolean canHandle(final String viewName) {
    ...
    return (((viewNamesToBeProcessed == null) || PatternMatchUtils.simpleMatch(viewNamesToBeProcessed, viewName))
        && ((viewNamesNotToBeProcessed == null) || !PatternMatchUtils.simpleMatch(viewNamesNotToBeProcessed,
            viewName)));
  }
  ...
  /**
   * Determines which internal method to call for creating the appropriate view. If no Broadleaf specific methods match
   * the viewName, it delegates to the parent ThymeleafViewResolver createView method
   *
   * @param   viewName  DOCUMENT ME!
   * @param   locale    DOCUMENT ME!
   *
   * @return  determines which internal method to call for creating the appropriate view.
   *
   * @throws  Exception  DOCUMENT ME!
   */
  @Override protected View createView(final String viewName, final Locale locale) throws Exception {
    if (!canHandle(viewName)) {
      LOG.trace("[THYMELEAF] View {" + viewName
        + "} cannot be handled by ThymeleafViewResolver. Passing on to the next resolver in the chain");
    ...
    if (viewName.startsWith(AJAX_REDIRECT_URL_PREFIX)) {
      LOG.trace("[THYMELEAF] View {" + viewName
        + "} is an ajax redirect, and will be handled directly by BroadleafThymeleafViewResolver");
  ...
  protected boolean isAjaxRequest() {
    if ((BroadleafRequestContext.getBroadleafRequestContext() != null)
          && (BroadleafRequestContext.getBroadleafRequestContext().getRequest() != null)) {
  ...
  /**
   * Performs a Broadleaf AJAX redirect. This is used in conjunction with BLC.js to support doing a browser page change
   * as as result of an AJAX call.
   *
   * @param   redirectUrl  DOCUMENT ME!
   * @param   locale       DOCUMENT ME!
   *
   * @return  performs a Broadleaf AJAX redirect.
   *
   * @throws  Exception
   */
  ...
  /**
   * @see  org.thymeleaf.spring3.view.ThymeleafViewResolver#loadView(java.lang.String, java.util.Locale)
   */
  @Override protected View loadView(final String originalViewName, final Locale locale) throws Exception {
...
} // end class BroadleafThymeleafViewResolver
