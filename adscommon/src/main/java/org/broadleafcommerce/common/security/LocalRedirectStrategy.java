...
/**
 * This class insures that if using the successUrl or failureUrl request parameter, then the urls are valid and are
 * local to the application (preventing a user modifying to go somewhere else on login success/failure).
 *
 * @author   $author$
...
public class LocalRedirectStrategy implements RedirectStrategy {
  ...
  /**
   * Set whether or not the context should be included in the redirect path. If true, the context is excluded from the
   * generated path, otherwise it is included.
   *
   * @param  contextRelative  DOCUMENT ME!
   */
  ...
  /**
   * This forces the redirect url port to match the request port. This could be problematic when switching between
   * secure and non-secure (e.g. http://localhost:8080 to https://localhost:8443)
   *
   * @param  enforcePortMatch  DOCUMENT ME!
   */
   ...
   *
   * @return  create the redirect url.
   */
   ...
   */
  private void validateRedirectUrl(String contextPath, String url, String requestServerName, int requestServerPort)
    ...
    String errorMessage =
      "Invalid redirect url specified.  Must be of the form /<relative view> or http[s]://<server name>[:<server port>][/<context path>]/...";
    logger.warn(errorMessage + ":  " + url);
    throw new MalformedURLException(errorMessage + ":  " + url);
  }

} // end class LocalRedirectStrategy
