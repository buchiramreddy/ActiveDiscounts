...
public class HibernateToolTask extends Task {
  ...
  /**
   * Creates a new HibernateToolTask object.
   */
  ...
  @Override public void execute() {
    ...
    try {
      ...
      LinkedHashMap<String, MergeFileSystemAndClassPathXMLApplicationContext.ResourceType> locations =
        new LinkedHashMap<String, MergeFileSystemAndClassPathXMLApplicationContext.ResourceType>();
      ...
      for (Task task : appContexts) {
        if (task instanceof ClassPathApplicationContextTask) {
          locations.put(((ClassPathApplicationContextTask) task).getPath(),
            MergeFileSystemAndClassPathXMLApplicationContext.ResourceType.CLASSPATH);
        } else if (task instanceof FileSystemApplicationContextTask) {
          locations.put(((FileSystemApplicationContextTask) task).getPath(),
            MergeFileSystemAndClassPathXMLApplicationContext.ResourceType.FILESYSTEM);
        }
      }
    ...
    try {
      for (Object configuration : configurationTasks) {
        ...
        while (iterator.hasNext()) {
          ...
          generatorTask.setOutputFileName(configurationTask.getDialect() + "_" + configurationTask.getPersistenceUnit()
            + ".sql");
   ...
   * DOCUMENT ME!
   *
   * @param  combinePersistenceUnits  DOCUMENT ME!
   */
   ...
   * DOCUMENT ME!
   *
   * @param  refineFileNames  DOCUMENT ME!
   */
  ...
  private String getProbableSolutionOrCause(Throwable re) {
    ...
    if (re instanceof UnsupportedClassVersionError) {
      return
        "You are most likely running the ant task with a JRE that is older than the JRE required to use the classes.\n"
    ...
    return null;
  } // end method getProbableSolutionOrCause
