...
public class SequenceGeneratorCorruptionDetection implements ApplicationListener<ContextRefreshedEvent> {
  ...
  /**
   * @see  org.springframework.context.ApplicationListener#onApplicationEvent(org.springframework.context.event.ContextRefreshedEvent)
   */
  @Override
  ...
  public void onApplicationEvent(ContextRefreshedEvent event) {
    if (detectSequenceGeneratorInconsistencies) {
      ...
      for (Object item : sessionFactory.getAllClassMetadata().values()) {
        ...
        if ((genericAnnot != null)
              && genericAnnot.strategy().equals("org.broadleafcommerce.common.persistence.IdOverrideTableGenerator")) {
        ...
        if (!StringUtils.isEmpty(segmentValue) && !StringUtils.isEmpty(tableName)
          ...
          if ((results2 != null) && !results2.isEmpty() && (results2.get(0) != null)) {
            ...
            LOG.info("Detecting id sequence state between " + mappedClass.getName() + " and " + segmentValue + " in "
              + tableName);
            ...
            if ((results != null) && !results.isEmpty() && (results.get(0) != null)) {
              ...
              if (maxEntityId > maxSequenceId) {
                if (automaticallyCorrectInconsistencies) {
                  ...
                  if (response <= 0) {
                    throw new RuntimeException("Unable to update " + tableName + " with the sequence generator id for "
                      + segmentValue);
                  }
                } else {
                  String reason = "A data inconsistency has been detected between the " + tableName
                    + " table and one or more entity tables for which it manages current max primary key values.\n"
                    + "The inconsistency was detected between the managed class (" + mappedClass.getName()
                    + ") and the identifier (" + segmentValue + ") in " + tableName + ". Broadleaf\n"
  ...
  } // end method onApplicationEvent

} // end class SequenceGeneratorCorruptionDetection
