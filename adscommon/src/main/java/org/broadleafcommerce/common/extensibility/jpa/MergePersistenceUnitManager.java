...
/**
 * Merges jars, class names and mapping file names from several persistence.xml files. The MergePersistenceUnitManager
 * will continue to keep track of individual persistence unit names (including individual data sources). When a specific
 * PersistenceUnitInfo is requested by unit name, the appropriate PersistenceUnitInfo is returned with modified jar
 * files urls, class names and mapping file names that include the comprehensive collection of these values from all
...
public class MergePersistenceUnitManager extends DefaultPersistenceUnitManager {
  ...
  /** DOCUMENT ME! */
  protected final boolean                   jpa2ApiPresent = ClassUtils.hasMethod(PersistenceUnitInfo.class,
      "getSharedCacheMode");
   ...
   */
  @PostConstruct public void configureClassTransformers() throws InstantiationException, IllegalAccessException,
   ...
   */
  @Override public PersistenceUnitInfo obtainDefaultPersistenceUnitInfo() {
    throw new IllegalStateException(
      "Default Persistence Unit is not supported. The persistence unit name must be specified at the entity manager factory.");
  }
   ...
   */
  @Override public PersistenceUnitInfo obtainPersistenceUnitInfo(String persistenceUnitName) {
  ...
  /**
   * @see  org.springframework.orm.jpa.persistenceunit.DefaultPersistenceUnitManager#preparePersistenceUnitInfos()
   */
  @Override
    ...
    try {
      ...
      for (Object pui : puis) {
        ...
        if (mPui.getPersistenceUnitRootUrl() == null) {
          Method determineDefaultPersistenceUnitRootUrl = getClass().getSuperclass().getDeclaredMethod(
        ...
        if (jpa2ApiPresent) {
          ...
          for (Class<?> clz : classes) {
            if ("org.springframework.orm.jpa.persistenceunit.DefaultPersistenceUnitManager$Jpa2PersistenceUnitInfoDecorator"
                  .equals(clz.getName())) {
              Constructor<?> constructor = clz.getConstructor(Class.forName(
      ...
      for (PersistenceUnitInfo pui : mergedPus.values()) {
        for (BroadleafClassTransformer transformer : classTransformers) {
          try {
            ...
            LOG.warn(
              "A BroadleafClassTransformer is configured for this persistence unit, but Spring reported a problem (likely that a LoadTimeWeaver is not registered). As a result, the Broadleaf Commerce ClassTransformer ("
              + transformer.getClass().getName() + ") is not being registered with the persistence unit.", e);
          }
        }
      }
  ...
  } // end method preparePersistenceUnitInfos
   ...
   * DOCUMENT ME!
   *
   * @param  classTransformers  DOCUMENT ME!
   */
   ...
   */
  @Override protected void postProcessPersistenceUnitInfo(MutablePersistenceUnitInfo newPU) {
  ...
  } // end method postProcessPersistenceUnitInfo

} // end class MergePersistenceUnitManager
