...
public class AnnotationsCopyClassTransformer implements BroadleafClassTransformer {
  ...
  /**
   * Creates a new AnnotationsCopyClassTransformer object.
   *
   ...
   * DOCUMENT ME!
   *
   * @param  xformTemplates  DOCUMENT ME!
   */
  ...
  @Override public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
    ...
    if (xformTemplates.containsKey(convertedClassName)) {
      ...
      logger.lifecycle(LifeCycleEvent.START,
        String.format("Transform - Copying annotations into [%s] from [%s]", xformKey,
      ...
      try {
        ...
        logger.lifecycle(LifeCycleEvent.END,
          String.format("Transform - Copying annotations into [%s] from [%s]", xformKey,
            StringUtils.join(xformVals, ",")));
   ...
   *
   * <p>We provide default implementations for List, Map, and Set, and will attempt to utilize a default constructor for
   * other classes.</p>
   *
   * <p>If the className contains an '[', we will return null.</p>
   *
   * @param   className  DOCUMENT ME!
   *
   * @return  this method will do its best to return an implementation type for a given classname.
   */
...
} // end class AnnotationsCopyClassTransformer
