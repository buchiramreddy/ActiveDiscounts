...
public class MergeClassPathXMLApplicationContext extends AbstractXmlApplicationContext {
  ...
  /**
   * Create a new MergeClassPathXMLApplicationContext, loading the definitions from the given definitions. Note, all
   * sourceLocation files will be merged using standard Spring configuration override rules. However, the patch files
   * are fully merged into the result of the sourceLocations simple merge. Patch merges are first executed according to
   * beans with the same id. Subsequent merges within a bean are executed against tagnames - ignoring any further id
   * attributes.
   *
   * @param   sourceLocations  array of relative (or absolute) paths within the class path for the source application
   *                           context files
   * @param   patchLocations   array of relative (or absolute) paths within the class path for the patch application
   *                           context files
   *
   ...
   */
  public MergeClassPathXMLApplicationContext(String[] sourceLocations, String[] patchLocations,
    ...
    for (int j = 0; j < sourceLocations.length; j++) {
      sources[j] = new ResourceInputStream(MergeClassPathXMLApplicationContext.class.getClassLoader()
          .getResourceAsStream(sourceLocations[j]), sourceLocations[j]);
    }

    ResourceInputStream[] patches = new ResourceInputStream[patchLocations.length];

    for (int j = 0; j < patches.length; j++) {
      patches[j] = new ResourceInputStream(MergeClassPathXMLApplicationContext.class.getClassLoader()
          .getResourceAsStream(patchLocations[j]), patchLocations[j]);
    }
    ...
    refresh();
  } // end ctor MergeClassPathXMLApplicationContext
...
} // end class MergeClassPathXMLApplicationContext
