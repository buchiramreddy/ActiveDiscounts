...
public class StandardConfigLocations {
  ...
  public static String[] retrieveAll(int contextType) throws IOException {
    ...
    try {
      reader = new BufferedReader(new InputStreamReader(
            StandardConfigLocations.class.getResourceAsStream("StandardConfigLocations.txt")));
  ...
  private static void addContextFile(int contextType, ArrayList<String> items, String temp) {
    if (!temp.startsWith("#") && (temp.trim().length() > 0)
          && (StandardConfigLocations.class.getClassLoader().getResource(temp.trim()) != null)) {
      if ((contextType == ALLCONTEXTTYPE)
            || (((contextType == WEBCONTEXTTYPE) || (contextType == APPCONTEXTTYPE)) && (temp.indexOf("-web-") >= 0))
            || (((contextType == SERVICECONTEXTTYPE) || (contextType == TESTCONTEXTTYPE)
                || (contextType == APPCONTEXTTYPE)) && (temp.indexOf("-web-") < 0) && (temp.indexOf("-test") < 0)
              && (temp.indexOf("-admin-") < 0))
            || (((contextType == SERVICECONTEXTTYPE) || (contextType == TESTCONTEXTTYPE)
                || (contextType == APPCONTEXTTYPE)) && (temp.indexOf("-admin-applicationContext-persistence") >= 0))
...
} // end class StandardConfigLocations
