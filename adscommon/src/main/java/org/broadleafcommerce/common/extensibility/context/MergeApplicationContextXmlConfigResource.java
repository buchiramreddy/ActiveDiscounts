...
public class MergeApplicationContextXmlConfigResource extends MergeXmlConfigResource {
  ...
  /**
   * Generate a merged configuration resource, loading the definitions from the given streams. Note, all sourceLocation
   * streams will be merged using standard Spring configuration override rules. However, the patch streams are fully
   * merged into the result of the sourceLocations simple merge. Patch merges are first executed according to beans with
   * the same id. Subsequent merges within a bean are executed against tagnames - ignoring any further id attributes.
   *
   ...
   *
   * @return  generate a merged configuration resource, loading the definitions from the given streams.
   *
   * @throws  org.springframework.beans.BeansException
   * @throws  FatalBeanException                        DOCUMENT ME!
   */
  public Resource[] getConfigResources(ResourceInputStream[] sources, ResourceInputStream[] patches)
  ...
  } // end method getConfigResources

} // end class MergeApplicationContextXmlConfigResource
