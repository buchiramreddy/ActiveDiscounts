 ...
 * PersistenceUnitManager allows us to pass in a list of PersistenceUnitPostProcessors. This class will allow us to
 * override or add custom JPA properties to the Persistence Unit. This is useful when different environments have
 * different requirements for JPA properties. The best example of this is SQL Dialect. You may want to use a dialect
 * such as Oracle for production and test environments, and perhaps HSQLDB for local and integration testing. You can
 * set the dialect property using the <code>RuntimeEnvironmentPropertiesConfigurer</code>. The keys will be the same in
 * each environment, but the values would be defined the environment-specific properties files. If you want the property
 * to be added only to certain environments, add the value "null" to the properties file. For example:
 *
 * <p><bean id="blPersistenceUnitManager"
 * class="org.broadleafcommerce.profile.extensibility.jpa.MergePersistenceUnitManager"> <property
 * name="persistenceXmlLocations"> <list> <value>classpath*:/META-INF/mycompany_persistence.xml</value> </list>
 * </property> <property name="persistenceUnitPostProcessors"> <list> <bean
 * class="org.broadleafcommerce.common.extensibility.jpa.JPAPropertiesPersistenceUnitPostProcessor"> <property
 * name="persistenceUnitProperties">
 *
 * <map>
 ...
 * </map>
 *
 * </property> </bean> </list> </property> </bean></p>
 *
 * @author   $author$
...
public class JPAPropertiesPersistenceUnitPostProcessor
  ...
  @PostConstruct public void populatePresetProperties() {
    if (!blPUHibernateHbm2ddlAuto.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.hbm2ddl.auto", blPUHibernateHbm2ddlAuto);
    }

    if (!blPUHibernateDialect.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.dialect", blPUHibernateDialect);
    }

    if (!blPUHibernateShow_sql.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.show_sql", blPUHibernateShow_sql);
    }

    if (!blPUHibernateCacheUse_second_level_cache.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.cache.use_second_level_cache",
        blPUHibernateCacheUse_second_level_cache);
    }

    if (!blPUHibernateCacheUse_query_cache.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.cache.use_query_cache", blPUHibernateCacheUse_query_cache);
    }

    if (!blPUHibernateHbm2ddlImport_files.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.hbm2ddl.import_files", blPUHibernateHbm2ddlImport_files);
    }

    if (!blPUHibernateHbm2ddlImport_files_sql_extractor.startsWith("${")) {
      persistenceUnitProperties.put("blPU.hibernate.hbm2ddl.import_files_sql_extractor",
        blPUHibernateHbm2ddlImport_files_sql_extractor);
    }

    if (!blCMSStorageHibernateHbm2ddlAuto.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.hbm2ddl.auto", blCMSStorageHibernateHbm2ddlAuto);
    }

    if (!blCMSStorageHibernateDialect.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.dialect", blCMSStorageHibernateDialect);
    }

    if (!blCMSStorageHibernateShow_sql.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.show_sql", blCMSStorageHibernateShow_sql);
    }

    if (!blCMSStorageHibernateCacheUse_second_level_cache.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.cache.use_second_level_cache",
        blCMSStorageHibernateCacheUse_second_level_cache);
    }

    if (!blCMSStorageHibernateCacheUse_query_cache.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.cache.use_query_cache",
        blCMSStorageHibernateCacheUse_query_cache);
    }

    if (!blCMSStorageHibernateHbm2ddlImport_files.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.hbm2ddl.import_files",
        blCMSStorageHibernateHbm2ddlImport_files);
    }

    if (!blCMSStorageHibernateHbm2ddlImport_files_sql_extractor.startsWith("${")) {
      persistenceUnitProperties.put("blCMSStorage.hibernate.hbm2ddl.import_files_sql_extractor",
        blCMSStorageHibernateHbm2ddlImport_files_sql_extractor);
    }

    if (!blSecurePUHibernateHbm2ddlAuto.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.hbm2ddl.auto", blSecurePUHibernateHbm2ddlAuto);
    }

    if (!blSecurePUHibernateDialect.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.dialect", blSecurePUHibernateDialect);
    }

    if (!blSecurePUHibernateShow_sql.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.show_sql", blSecurePUHibernateShow_sql);
    }

    if (!blSecurePUHibernateCacheUse_second_level_cache.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.cache.use_second_level_cache",
        blSecurePUHibernateCacheUse_second_level_cache);
    }

    if (!blSecurePUHibernateCacheUse_query_cache.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.cache.use_query_cache",
        blSecurePUHibernateCacheUse_query_cache);
    }

    if (!blSecurePUHibernateHbm2ddlImport_files.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.hbm2ddl.import_files",
        blSecurePUHibernateHbm2ddlImport_files);
    }

    if (!blSecurePUHibernateHbm2ddlImport_files_sql_extractor.startsWith("${")) {
      persistenceUnitProperties.put("blSecurePU.hibernate.hbm2ddl.import_files_sql_extractor",
        blSecurePUHibernateHbm2ddlImport_files_sql_extractor);
    }

    persistenceUnitProperties.putAll(overrideProperties);
  } // end method populatePresetProperties
   ...
   */
  @Override public void postProcessPersistenceUnitInfo(MutablePersistenceUnitInfo pui) {
...
} // end class JPAPropertiesPersistenceUnitPostProcessor
