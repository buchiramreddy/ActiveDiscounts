...
public class Money implements Serializable, Cloneable, Comparable<Money>, Externalizable {
    ...
    /**
     * Attempts to load a default currency by using the default locale. {@link java.util.Currency#getInstance(java.util.Locale)} uses the country component of the locale to resolve the currency. In some instances, the locale may not have a country component, in which case the default currency can be controlled with a
     * system property.
